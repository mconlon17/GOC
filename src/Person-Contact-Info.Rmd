...
```{r organizations}
tab <- get.contacts() %>%
  filter(c2_contact_record_type_c == "Person") %>%
  select(
    contact2_full_name, c2_primary_phone, email_address, primary_address_street, primary_address_city,
    primary_address_state, primary_address_postalcode, last_name, first_name
  ) %>%
...
ft <- goc.table(tab, "Organization Contact Info")
ft
```
