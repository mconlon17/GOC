...
get.members <- function(include.deleted = FALSE, active.only = FALSE) {
  ...
  if (active.only) {
    recent.members <- members %>%
      ...
      inner_join(get.attendance(), by = c("contact_id_c" = "contact_2_id")) %>% # Inner join must be a Member with attendance
      mutate(Since = Sys.Date() - as_date(da_date_of_attendance)) %>%
      rename(id = id.x) %>%
      group_by(id) %>%
      summarise(MostRecent = min(Since)) %>%
      filter(MostRecent <= 90) %>%
      select(id)
    members <- members %>%
...
get.attendance <- function(include.deleted = FALSE) {
